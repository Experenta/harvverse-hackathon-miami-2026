...
```json
{
  ...
  "scripts": {
    ...
    "local-ship": "pnpm abi-copy && pnpm codegen && pnpm build --network localhost && pnpm deploy-local",
    "test": "graph test -d",
...
```json
{
  "graph": "pnpm --filter @se-2/subgraph graph",
  "graphclient:build": "pnpm --filter @se-2/nextjs client",
  "subgraph:abi-copy": "pnpm --filter @se-2/subgraph abi-copy",
  "subgraph:build": "pnpm --filter @se-2/subgraph build",
  "subgraph:clean-node": "pnpm --filter @se-2/subgraph clean-node",
  "subgraph:codegen": "pnpm --filter @se-2/subgraph codegen",
  "subgraph:create-local": "pnpm --filter @se-2/subgraph create-local",
  "subgraph:local-ship": "pnpm --filter @se-2/subgraph local-ship",
  "subgraph:run-node": "pnpm --filter @se-2/subgraph run-node",
  "subgraph:stop-node": "pnpm --filter @se-2/subgraph stop-node",
  "subgraph:test": "pnpm --filter @se-2/subgraph test -d"
}
```
...
After running `pnpm graphclient:build`, import the generated client. Use TanStack Query (already available in SE-2) for data fetching:
...
> **`~~/.graphclient`** is the generated runtime artifact. It only exists after `pnpm graphclient:build`. The `.graphclient/` directory should NOT be committed — it's generated from `.graphclientrc.yml` and the GQL files.

## Gotchas & Common Pitfalls

**Docker must be running.** The local Graph Node, IPFS, and Postgres all run in Docker. If Docker isn't running, `pnpm subgraph:run-node` will fail.

**`pnpm deploy` must run before `pnpm subgraph:abi-copy`.** The ABI copy script reads from `deployedContracts.ts` which is generated by the deploy step. If you haven't deployed, there's nothing to copy.
...
**Graph Client artifacts must be regenerated after schema changes.** Run `pnpm graphclient:build` whenever you change the GraphQL schema or queries. The frontend imports from `~~/.graphclient` which contains generated types.
...
1. `pnpm chain` — start local blockchain
2. `pnpm deploy` — deploy contracts (generates `deployedContracts.ts`)
3. `pnpm subgraph:run-node` — start Docker Graph Node (keep this terminal open)
4. `pnpm subgraph:create-local` — register subgraph (once only)
5. `pnpm subgraph:local-ship` — copies ABIs, generates types, builds, and deploys
6. Visit `http://localhost:8000/subgraphs/name/scaffold-eth/your-contract/graphql` — test GraphQL queries
7. `pnpm graphclient:build` — generate frontend client artifacts
8. `pnpm start` — visit the subgraph page to see indexed data
9. `pnpm subgraph:test` — run Matchstick unit tests
...
3. `pnpm graph auth --studio <DEPLOY_KEY>`
4. `pnpm graph deploy --studio <SUBGRAPH_SLUG>`
